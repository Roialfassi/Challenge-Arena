...
const LN={1:"Absolute Basics",2:"Simple Processing",3:"While Loops & Input",4:"Intermediate Core",5:"Intermediate Creative",6:"Intermediate+",7:"Advanced",8:"Advanced+",9:"Expert",10:"Master"};
const TAGS=[...new Set(C.flatMap(c=>c.tags))].sort();

// ═══ SIDEBAR ═══
function buildSidebar(){
  let h=`<div class="sb-section"><div class="sb-label">Search</div><input class="sb-search" type="text" placeholder="Type to filter..." id="sSearch"></div>`;
  h+=`<div class="sb-section"><div class="sb-label">Topics</div>`;
  h+=`<div class="sb-item active" data-tag="all">All<span class="cnt">${C.length}</span></div>`;
  TAGS.forEach(t=>{
    const n=C.filter(c=>c.tags.includes(t)).length;
    h+=`<div class="sb-item" data-tag="${t}">${t.replace('-',' ')}<span class="cnt">${n}</span></div>`;
  });
  h+=`</div>`;
  h+=`<div class="sb-section"><div class="sb-label">Level</div>`;
  LEVELS.forEach(l=>{
    const n=C.filter(c=>c.l===l).length;
    h+=`<div class="sb-level" data-lv="${l}"><span class="lnum">${l}</span>${LN[l]}<span class="cnt" style="margin-left:auto">${n}</span></div>`;
  });
  h+=`</div>`;
  ...
  document.querySelectorAll('.sb-level[data-lv]').forEach(el=>{
    el.addEventListener('click',()=>{
      ...
      if(fLv===lv){fLv=null;el.classList.remove('active');}
      else{document.querySelectorAll('.sb-level').forEach(e=>e.classList.remove('active'));el.classList.add('active');fLv=lv;}
      applyF();
    });
  });
  document.getElementById('sSearch').addEventListener('input',e=>{fQ=e.target.value.toLowerCase();applyF();});
}
...
function renderCh(c){
  const tagH=c.tags.map(t=>`<span class="tag tag-${t}">${t.replace('-',' ')}</span>`).join('');
  const hasC=c.ex.some(e=>e.c);
  const exH=`<table class="ex-tbl"><tr><th>Input</th><th>Output</th>${hasC?'<th></th>':''}</tr>`+
    c.ex.map(e=>`<tr><td>${e.i}</td><td>${e.o}</td>${hasC?`<td class="cmt">${e.c||''}</td>`:''}</tr>`).join('')+`</table>`;
  const conH=c.con.length?`<ul class="const-list">${c.con.map(x=>`<li>${x}</li>`).join('')}</ul>`:'';
  const scH=c.sc?`<div class="ch-scenario">${c.sc}</div>`:'';

  return `<div class="ch" data-id="${c.id}" data-tags="${c.tags.join(',')}" data-lv="${c.l}">
  <div class="ch-header">
    <div class="ch-lvl" data-l="${c.l}">${c.l}</div>
    <div class="ch-info">
      <div class="ch-title">${c.t}</div>
      <div class="ch-meta">${tagH}</div>
      <div class="ch-desc-preview">${c.d}</div>
    </div>
    <div class="ch-arrow">&#9662;</div>
  </div>
  <div class="ch-body">
    ${scH}
    <div class="ch-fulldesc">${c.d}</div>
    <div class="detail-label">Function Signature</div>
    <div class="sig-code">${c.sig}</div>
    <div class="detail-label">Examples</div>
    ${exH}
    ${conH}
    <div class="hint-toggle" onclick="event.stopPropagation();const b=this.nextElementSibling;b.classList.toggle('show');this.textContent=b.classList.contains('show')?'Hide Hint':'Show Hint';">Show Hint</div>
...
function applyF(){
  ...
  document.querySelectorAll('.ch').forEach(el=>{
    ...
    if(fQ){const s=(c.t+' '+c.d+' '+(c.sc||'')+' '+c.tags.join(' ')).toLowerCase();if(!s.includes(fQ))show=false;}
    el.classList.toggle('hidden',!show);
    if(show)vis++;
  });
  document.getElementById('resultCount').textContent=vis+' challenges';
  document.getElementById('emptyState').style.display=vis===0?'':'none';
  let title='All Challenges';
  if(fLv)title=`Level ${fLv}: ${LN[fLv]}`;
  if(fTag!=='all')title=fTag.replace('-',' ').replace(/\b\w/g,c=>c.toUpperCase())+' Challenges';
...
function buildPracticeGrid(){
  let h='';
  LEVELS.forEach(l=>{h+=`<div class="level-pick" onclick="pStart(${l})"><div class="lp-num">${l}</div><div class="lp-name">${LN[l]}</div></div>`;});
  document.getElementById('levelGrid').innerHTML=h;
}
function pStart(l){pLv=l;pSeen.clear();pN=0;pShow();}
function pShow(){
  let pool=C.filter(c=>c.l===pLv&&!pSeen.has(c.id));
  if(!pool.length){C.filter(c=>c.l===pLv).forEach(c=>pSeen.delete(c.id));pool=C.filter(c=>c.l===pLv);}
  const ch=pool[Math.floor(Math.random()*pool.length)];
  pSeen.add(ch.id);pN++;
  document.getElementById('pracStart').style.display='none';
  document.getElementById('pracActive').classList.add('show');
  document.getElementById('pracBadge').textContent=`Level ${pLv} — ${LN[pLv]}`;
  document.getElementById('pracCountEl').textContent=`Challenge #${pN}`;
  ...
  // Add title
  const titleH=`<div style="display:flex;align-items:center;gap:0.7rem;margin-bottom:0.8rem;">
    <div class="ch-lvl" data-l="${ch.l}" style="width:38px;height:38px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-family:JetBrains Mono;font-weight:700;font-size:0.95rem;color:white;">${ch.l}</div>
    <div><div style="font-weight:700;font-size:1.05rem;color:var(--deep);">${ch.t}</div>
    <div class="ch-meta">${ch.tags.map(t=>`<span class="tag tag-${t}">${t.replace('-',' ')}</span>`).join('')}</div></div>
  </div>`;
...
function pracGo(d){if(d===-1&&pLv>1)pLv--;else if(d===1&&pLv<10)pLv++;pShow();}
function pracReset(){pLv=null;pSeen.clear();pN=0;document.getElementById('pracStart').style.display='';document.getElementById('pracActive').classList.remove('show');}

// ═══ INIT ═══
buildSidebar();buildList();buildPracticeGrid();applyF();
